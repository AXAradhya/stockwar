...
export const TeamCloud9Modal: React.FC<TeamCloud9ModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div
        className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-cyan-500/30 bg-[#0a0d14]/95 p-6 shadow-2xl shadow-cyan-500/20 backdrop-blur-xl transition-all transform scale-100"
        style={{ animation: 'teamModalPop 0.25s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}
      >
        {/* Style tag for custom pop animation */}
        <style>{`
